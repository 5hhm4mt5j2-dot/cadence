...
export default function TopBar({ v }) {
  return (
    <div style={s('flex:none; display:flex; align-items:center; justify-content:space-between; padding:14px 16px 4px;')}>
      <button onClick={v.openMenu} data-tut="menu" aria-label="Menu" style={s('width:40px; height:40px; border-radius:12px; border:1px solid var(--border); background:var(--surface); cursor:pointer; display:flex; align-items:center; justify-content:center; flex:none; color:var(--text); padding:0;')}>
      ...
      <div style={s('display:flex; align-items:center; gap:6px; flex:none;')}>
        {v.showAmend && (
          <button onClick={v.amendAction} aria-label="Edit day" style={s('height:40px; padding:0 15px; display:inline-flex; align-items:center; gap:6px; border:1px solid var(--accent); cursor:pointer; background:var(--accent-soft); color:var(--accent); border-radius:12px; font-family:inherit; font-size:12.5px; font-weight:800; transition:transform .12s cubic-bezier(.34,1.56,.64,1);')}>
        ...
        {v.onWeek && (
          <button onClick={v.toggleTheme} aria-label="Toggle dark mode" style={s('width:40px; height:40px; border-radius:12px; border:1px solid var(--border); background:var(--surface); cursor:pointer; display:flex; align-items:center; justify-content:center; padding:0; color:var(--text); flex:none;')}>
        ...
        {v.notWeek && (
          <button onClick={v.topAction} aria-label="Back" style={s('width:40px; height:40px; display:inline-flex; align-items:center; justify-content:center; border:1px solid var(--border); cursor:pointer; background:var(--surface); color:var(--text); border-radius:12px; padding:0;')}>
